...
private:
  ...
  _CCCL_DEVICE_API _CCCL_FORCEINLINE void scan_and_choose(histo_counter_t k)
  {
    ...
    for (int i = 0; i < buckets_per_thread; ++i)
    {
      ...
      {
        counts[i] = storage.histogram[bin_idx];
      }
    }
    ...
    DecomposerT decomposer = DecomposerT{})
  {
    ...
    for (int pass = 0; pass < num_passes; ++pass)
    {
      ...
      scan_and_choose(static_cast<histo_counter_t>(k));
      __syncthreads();
