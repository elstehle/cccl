...
public:
  ...
  template <bool IsFullTile, bool BlockedInput = true, bool UntwiddleKeys = true, int ItemsPerThread>
  ...
  select_max(KeyT (&keys)[ItemsPerThread], int k, int valid_items, int begin_bit = 0, int end_bit = sizeof(KeyT) * 8)
  {
    ...
    if (states.has_ties())
    {
      algorithm_t(storage_).template refine_keys<detail::topk::select::max, IsFullTile, UntwiddleKeys>(
  ...
  select_min(KeyT (&keys)[ItemsPerThread], int k, int valid_items, int begin_bit = 0, int end_bit = sizeof(KeyT) * 8)
  {
    ...
    if (states.has_ties())
    {
      algorithm_t(storage_).template refine_keys<detail::topk::select::min, IsFullTile, UntwiddleKeys>(
        keys, states, begin_bit, end_bit);
    }
    return states;
  }
  ...
  //! @tparam ItemsPerThread Number of items per thread; deduced from @p keys
  //!                       / @p states.
  template <bool UntwiddleKeys = true, int ItemsPerThread>
    ...
    int end_bit   = sizeof(KeyT) * 8)
  {
    ...
    if (states.has_ties())
    {
      algorithm_t(storage_).template refine_keys<detail::topk::select::max, false, UntwiddleKeys>(keys, states, begin_bit, end_bit);
    }
  }
    ...
    int end_bit   = sizeof(KeyT) * 8)
  {
    ...
    if (states.has_ties())
    {
      algorithm_t(storage_).template refine_keys<detail::topk::select::min, false, UntwiddleKeys>(keys, states, begin_bit, end_bit);
    }
  }
};
...
public:
  ...
  //! Zero the ticket counters ahead of `rank_key_states`; must be separated from it by at
  //! least one `__syncthreads()` (e.g. the sieve stage's internal barriers).
  _CCCL_DEVICE_API _CCCL_FORCEINLINE void prime()
  {
    algorithm_t(storage_).prime();
  }
