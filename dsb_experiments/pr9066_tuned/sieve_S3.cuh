...
private:
  ...
  struct TempStorage_
  {
    ...
    histo_counter_t pass_state_packed; // bucket<<22 | candidates<<11 | selected (tile<=2047)
  };
  ...
  _CCCL_DEVICE_API _CCCL_FORCEINLINE void init_histograms()
  {
    ...
    {
      storage.histogram[0][histo_offset + linear_tid] = 0;
      storage.histogram[1][histo_offset + linear_tid] = 0;
    }
  ...
  // Fused prefix sum + k-th bucket selection (no histogram writeback, no separate choose phase)
  _CCCL_DEVICE_API _CCCL_FORCEINLINE void scan_and_choose(const histo_counter_t (&histogram)[num_buckets], histo_counter_t k)
  {
    ...
    for (int i = 0; i < buckets_per_thread; ++i)
    {
      ...
      if (bin_idx < num_buckets)
      {
        ...
        if (excl < k && incl >= k)
        {
          storage.pass_state_packed = (histo_counter_t(bin_idx) << 22) | ((incl - excl) << 11) | excl;
        }
      }
    }
  }
    ...
    DecomposerT decomposer = DecomposerT{})
  {
    ...
    // Zero both histogram buffers once up front
    init_histograms();
    __syncthreads();
    ...
    for (int pass = 0; pass < num_passes; ++pass)
    {
      ...
      if (pass > 0 && pass + 1 < num_passes)
      {
        int off = 0;
        _CCCL_PRAGMA_UNROLL_FULL()
        for (; off + threads_per_block <= num_buckets; off += threads_per_block)
        {
          nxt[off + linear_tid] = 0;
        }
      }
      __syncthreads();
      ...
      // Update the current k and length for the next pass (single packed state load)
      const histo_counter_t st = storage.pass_state_packed;
      const int st_selected    = int(st & 0x7ffu);
      const int st_cands       = int((st >> 11) & 0x7ffu);
      const int st_bucket      = int(st >> 22);
      k -= st_selected;
      num_candidates = st_cands;
      total_selected += st_selected;
      ...
      const auto kth_key_digit =
        (SelectDirection == detail::topk::select::min) ? st_bucket : (num_buckets - 1 - st_bucket);
