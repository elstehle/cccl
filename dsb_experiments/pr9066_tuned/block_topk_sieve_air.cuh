...
// PR #9066's block_topk_sieve_air with the dsb_experiments latency optimizations applied
// (see BLOCK_TOPK_AIR_OPT_RESULTS.md):
//   * double-buffered histograms: both buffers are zeroed once up front; the buffer for pass
//     p+1 is re-zeroed during pass p's histogram phase (it was last read a barrier earlier),
//     removing the per-pass init phase and its barrier
//   * fused scan+choose (resolves the in-code TODO): the crossing test runs on the scan's
//     register results (exclusive = inclusive - count); no histogram writeback, no separate
//     choose phase, one barrier fewer per pass
//   (a packed single-word pass state was evaluated and REGRESSED ~250 cyc/pass here; the
//   three-field state is kept)
//   * optional UntwiddleKeys=false on refine_keys: callers that keep the original keys (e.g.
//     the composed block_topk_air scattering original values) skip the flip-back tracking and
//     the un-twiddle pass entirely; keys are left bit-twiddled on return
...
private:
  ...
  // Zero one histogram buffer (loop unrolling is beneficial for performance here)
  ...
  // Fused prefix sum + k-th bucket selection: the crossing test runs on the scan's register
  // results directly (exclusive = inclusive - count); no histogram writeback, no separate
  // choose phase. Resolves the former TODO in compute_bin_offsets.
  _CCCL_DEVICE_API _CCCL_FORCEINLINE void
  scan_and_choose(const histo_counter_t (&histogram)[num_buckets], histo_counter_t k)
  {
    ...
    histo_counter_t sums[buckets_per_thread];
    _CCCL_PRAGMA_UNROLL_FULL()
    for (int i = 0; i < buckets_per_thread; ++i)
    {
      sums[i] = counts[i];
    }
    block_scan_t(storage.scan_temp_storage).InclusiveSum(sums, sums);
    ...
    for (int i = 0; i < buckets_per_thread; ++i)
    {
      ...
      if (bin_idx < num_buckets)
      {
        const histo_counter_t incl = sums[i];
        const histo_counter_t excl = incl - counts[i];
        _CCCL_ASSERT((bin_idx != num_buckets - 1) || (incl >= k),
                     "Less than k candidates have participated in the histogram");
        if (excl < k && incl >= k)
        {
          storage.pass_state.bucket     = bin_idx;
          storage.pass_state.candidates = incl - excl;
          storage.pass_state.selected   = excl;
        }
      }
    }
  }
    ...
    DecomposerT decomposer = DecomposerT{})
  {
    ...
    // Zero both histogram buffers once; pass p+1's buffer is re-zeroed during pass p's
    // histogram phase (its last read precedes pass p's state barrier)
    zero_histogram(storage.histogram[0]);
    zero_histogram(storage.histogram[1]);
    __syncthreads();
    ...
    for (int pass = 0; pass < num_passes; ++pass)
    {
      ...
      histo_counter_t(&cur)[num_buckets] = storage.histogram[pass & 1];
      histo_counter_t(&nxt)[num_buckets] = storage.histogram[(pass + 1) & 1];

      // Compute histogram over the current pass's bits, pre-filtered for keys matching the previous
      // pass's prefix mask; zero the other buffer for the next pass in the same phase
      ...
      compute_histograms<SelectDirection, IsFullTile>(unsigned_keys, states, digit_extractor, filter_op, cur);
      ...
      // Fused prefix sum + k-th bucket selection
      scan_and_choose(cur, static_cast<histo_counter_t>(k));
      __syncthreads();
  ...
  template <detail::topk::select SelectDirection, bool IsFullTile, bool UntwiddleKeys, int ItemsPerThread>
  _CCCL_DEVICE_API _CCCL_FORCEINLINE void
  select_topk(KeyT (&keys)[ItemsPerThread], block_topk_key_states<ItemsPerThread>& states, int begin_bit, int end_bit)
  {
    ...
    // restore -0.0 in the output via a bitvector; no extra key buffer. When the caller keeps the original keys
    // (UntwiddleKeys == false), the flip tracking and the un-twiddle pass are skipped entirely.
...
public:
  ...
  //! @tparam UntwiddleKeys If false, @p keys are left bit-twiddled on return (the flip-back
  //!                       tracking and the un-twiddle pass are skipped); use when the caller
  //!                       keeps the original keys, e.g. to scatter original values.
  template <detail::topk::select SelectDirection, bool IsFullTile, bool UntwiddleKeys = true, int ItemsPerThread>
  ...
  {
    select_topk<SelectDirection, IsFullTile, UntwiddleKeys>(keys, states, begin_bit, end_bit);
  }
};
} // namespace detail
