...
// Packed-u64 hierarchical extraction using the two-stage redux 64-bit argmax (TRT-LLM trick):
// each warp round is an EXACT argmax of (value<<32 | index) in ~54 cyc — no ballot, no tie
// machinery; a fixed lane records the winner off the critical path.
struct ProtoHierTwoRedux
{
  static constexpr const char* name = "hier_2xredux (packed u64, exact argmax)";
  static constexpr bool is_half     = false;
  struct Smem
  {
    unsigned long long cv[128];
  };
  __device__ __forceinline__ static unsigned long long wmax64(unsigned long long c)
  {
    unsigned hi = (unsigned) (c >> 32), lo = (unsigned) c, mh, ml;
    asm volatile("redux.sync.max.u32 %0, %1, 0xffffffff;" : "=r"(mh) : "r"(hi));
    const unsigned loC = (hi == mh) ? lo : 0u;
    asm volatile("redux.sync.max.u32 %0, %1, 0xffffffff;" : "=r"(ml) : "r"(loC));
...
#pragma unroll
    for (int r = 0; r < kK; ++r)
    {
      const unsigned long long M = wmax64(p[0]);
      const bool w               = (p[0] == M); // unique winner: index in the low bits
      p[0]                       = w ? p[1] : p[0];
      p[1]                       = w ? p[2] : p[1];
      p[2]                       = w ? p[3] : p[2];
      p[3]                       = w ? 0ull : p[3];
...
#pragma unroll
      for (int r = 0; r < kK; ++r)
      {
        const unsigned long long M = wmax64(q[0]);
        const bool w               = (q[0] == M);
        q[0]                       = w ? q[1] : q[0];
        q[1]                       = w ? q[2] : q[1];
        q[2]                       = w ? q[3] : q[2];
        ...
        if (lane == 0)
        {
          out_v[r] = untwiddle((unsigned) (M >> 32));
          out_i[r] = (int) (M & 0xffffffffu);
        }
      }
    }
    __syncthreads();
